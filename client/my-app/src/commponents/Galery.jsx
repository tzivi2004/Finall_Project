...
export default function AdvancedDemo() {
    ...
    const fetchImages = async () => {
        const res = await Axios.get("http://localhost:1233/api/images");
        const imagesForGalleria = res.data.map(filename => ({
            itemImageSrc: `http://localhost:1233/uploads/${filename}`,
            thumbnailImageSrc: `http://localhost:1233/uploads/${filename}`,
            alt: filename,
            title: filename}));
        setImages(imagesForGalleria);
        console.log("images", imagesForGalleria);
    };
    fetchImages();
    ...
    return (
        <div className="card galleria-demo"style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', minHeight: '80vh' }}>
